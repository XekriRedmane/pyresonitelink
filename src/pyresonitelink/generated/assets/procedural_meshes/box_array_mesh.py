...
Note: ``Points``, ``Sizes``, ``Rotations``, ``Colors``, and ``UV_Scales`` arrays must be the same length for this component to work.
    ...
    Use a brush tool from ``ResoniteEssentials/Tools/Brushes/Box Brushes``
    and draw to make one of these with values already initialized.
    """
    ...
    def colors_profile(self) -> ColorProfile | None:
        """__TPL__. However, this is for every box in the array."""
    ...
    def colors_profile(self, value: ColorProfile | str) -> None:
        """Set ColorsProfile. __TPL__. However, this is for every box in the array."""
        member = self.get_member("ColorsProfile")
        if isinstance(member, members.FieldEnum):
            member.value = str(value)
        else:
