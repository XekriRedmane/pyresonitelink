...
class TransformDirection(GeneratedComponent, INodeValueOutput, IExecutionNode, INode, ICustomInspector, IObjectRoot, IWorldEventReceiver):
    """Wrapper for [ProtoFluxBindings]FrooxEngine.ProtoFlux.Runtimes.Execution.Nodes.FrooxEngine.Transform.TransformDirection.

    Category: ProtoFlux/Runtimes/Execution/Nodes/Transform/Conversion
    """

    COMPONENT_TYPE = "[ProtoFluxBindings]FrooxEngine.ProtoFlux.Runtimes.Execution.Nodes.FrooxEngine.Transform.TransformDirection"

    def __init__(self, from_space: str | INodeObjectOutput[Slot] | None = None, to_space: str | INodeObjectOutput[Slot] | None = None, source_direction: str | INodeValueOutput[primitives.Float3] | None = None, *, component: workers.Component | None = None) -> None:
        """Initialize with optional member values.

        Args:
            from_space: Initial value for FromSpace.
            to_space: Initial value for ToSpace.
            source_direction: Initial value for SourceDirection.
    ...
    def source_direction(self) -> str | None:
        """Target ID of the SourceDirection reference (targets INodeValueOutput[primitives.Float3])."""
        member = self.get_member("SourceDirection")
        if isinstance(member, members.Reference):
            return member.targetId
        return None

    @source_direction.setter
    def source_direction(self, target: str | INodeValueOutput[primitives.Float3] | None) -> None:
        """Set the SourceDirection reference by target ID or INodeValueOutput[primitives.Float3] instance."""
