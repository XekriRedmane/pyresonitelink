...
class TransformScale(GeneratedComponent, INodeValueOutput, IExecutionNode, INode, ICustomInspector, IObjectRoot, IWorldEventReceiver):
    """Wrapper for [ProtoFluxBindings]FrooxEngine.ProtoFlux.Runtimes.Execution.Nodes.FrooxEngine.Transform.TransformScale.

    Category: ProtoFlux/Runtimes/Execution/Nodes/Transform/Conversion
    """

    COMPONENT_TYPE = "[ProtoFluxBindings]FrooxEngine.ProtoFlux.Runtimes.Execution.Nodes.FrooxEngine.Transform.TransformScale"

    def __init__(self, from_space: str | INodeObjectOutput[Slot] | None = None, to_space: str | INodeObjectOutput[Slot] | None = None, source_scale: str | INodeValueOutput[primitives.Float3] | None = None, *, component: workers.Component | None = None) -> None:
        """Initialize with optional member values.

        Args:
            from_space: Initial value for FromSpace.
            to_space: Initial value for ToSpace.
            source_scale: Initial value for SourceScale.
    ...
    @property
    def source_scale(self) -> str | None:
        """Target ID of the SourceScale reference (targets INodeValueOutput[primitives.Float3])."""
        member = self.get_member("SourceScale")
        if isinstance(member, members.Reference):
            return member.targetId
        return None

    @source_scale.setter
    def source_scale(self, target: str | INodeValueOutput[primitives.Float3] | None) -> None:
        """Set the SourceScale reference by target ID or INodeValueOutput[primitives.Float3] instance."""
