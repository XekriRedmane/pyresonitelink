...
class AssignRole(GeneratedComponent, ISyncNodeOperation, IExecutionNode, INode, ICustomInspector, IObjectRoot, IWorldEventReceiver):
    ...
    def __init__(self, next: str | INodeOperation | None = None, handle: str | INodeObjectOutput[JoinRequestHandle] | None = None, role_name: str | INodeObjectOutput[str] | None = None, *, component: workers.Component | None = None) -> None:
    ...
    def role_name(self) -> str | None:
        """Target ID of the RoleName reference (targets INodeObjectOutput[str])."""
    ...
    @role_name.setter
    def role_name(self, target: str | INodeObjectOutput[str] | None) -> None:
        """Set the RoleName reference by target ID or INodeObjectOutput[str] instance."""
        target_id: str | None = target.id if isinstance(target, INodeObjectOutput) else target  # type: ignore[assignment]
        member = self.get_member("RoleName")
        if isinstance(member, members.Reference):
            member.targetId = target_id
