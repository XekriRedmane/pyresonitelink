...
class FormatLocaleString(GeneratedComponent, INodeObjectOutput, IExecutionNode, INode, ICustomInspector, IObjectRoot, IWorldEventReceiver):
    ...
    def __init__(self, locale: str | INodeObjectOutput[LocaleResource] | None = None, key: str | INodeObjectOutput[str] | None = None, *, component: workers.Component | None = None) -> None:
    ...
    def key(self) -> str | None:
        """Target ID of the Key reference (targets INodeObjectOutput[str])."""
    ...
    @key.setter
    def key(self, target: str | INodeObjectOutput[str] | None) -> None:
        """Set the Key reference by target ID or INodeObjectOutput[str] instance."""
        target_id: str | None = target.id if isinstance(target, INodeObjectOutput) else target  # type: ignore[assignment]
        member = self.get_member("Key")
        if isinstance(member, members.Reference):
            member.targetId = target_id
