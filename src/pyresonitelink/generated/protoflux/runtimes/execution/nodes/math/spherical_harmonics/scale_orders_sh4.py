...
class ScaleOrdersSH4(GenericComponent[T], INodeValueOutput[T], IExecutionNode[T], INode, ICustomInspector, IObjectRoot, IWorldEventReceiver):
    """The Scale Orders SH4 node takes in a SphericalHarmonicsL4 along with a set of orders that refine/modify/scale it, then returns the new Spherical Harmonic of the same type.

    Category: ProtoFlux/Runtimes/Execution/Nodes/Math/Spherical Harmonics

    Parameterize with a value type::

        ScaleOrdersSH4[primitives.Float]
    ...
    def order4(self) -> str | None:
        """Target ID of the Order4 reference (targets INodeValueOutput[primitives.Float])."""
    ...
    @order4.setter
    def order4(self, target: str | INodeValueOutput[primitives.Float] | None) -> None:
        """Set the Order4 reference by target ID or INodeValueOutput[primitives.Float] instance."""
        target_id: str | None = target.id if isinstance(target, INodeValueOutput) else target  # type: ignore[assignment]
        member = self.get_member("Order4")
        if isinstance(member, members.Reference):
            member.targetId = target_id
