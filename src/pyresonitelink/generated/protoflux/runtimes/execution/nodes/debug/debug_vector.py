...
class DebugVector(GeneratedComponent, IMappableNode, ISyncNodeOperation, IExecutionNode, INode, ICustomInspector, IObjectRoot, IWorldEventReceiver):
    """Wrapper for [ProtoFluxBindings]FrooxEngine.ProtoFlux.Runtimes.Execution.Nodes.FrooxEngine.Debugging.DebugVector.
    ...
    def vector(self) -> str | None:
        """Target ID of the Vector reference (targets INodeValueOutput[primitives.Float3])."""
    ...
    def radius_ratio(self) -> str | None:
        """Target ID of the RadiusRatio reference (targets INodeValueOutput[np.float32])."""
    ...
    @radius_ratio.setter
    def radius_ratio(self, target: str | INodeValueOutput[np.float32] | None) -> None:
        """Set the RadiusRatio reference by target ID or INodeValueOutput[np.float32] instance."""
        target_id: str | None = target.id if isinstance(target, INodeValueOutput) else target  # type: ignore[assignment]
        member = self.get_member("RadiusRatio")
        if isinstance(member, members.Reference):
            member.targetId = target_id
