...
class TryEnumToUshort(GenericComponent[T], INodeValueOutput[T], IExecutionNode[T], INode, ICustomInspector, IObjectRoot, IWorldEventReceiver):
    ...
    def __init__(self, value: str | INodeValueOutput[E] | None = None, fail_value: str | INodeValueOutput[np.uint16] | None = None, *, component: workers.Component | None = None) -> None:
    ...
    def fail_value(self) -> str | None:
        """Target ID of the FailValue reference (targets INodeValueOutput[np.uint16])."""
    ...
    @fail_value.setter
    def fail_value(self, target: str | INodeValueOutput[np.uint16] | None) -> None:
        """Set the FailValue reference by target ID or INodeValueOutput[np.uint16] instance."""
        target_id: str | None = target.id if isinstance(target, INodeValueOutput) else target  # type: ignore[assignment]
        member = self.get_member("FailValue")
        if isinstance(member, members.Reference):
            member.targetId = target_id
