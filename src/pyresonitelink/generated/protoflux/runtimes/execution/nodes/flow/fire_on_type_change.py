...
class FireOnTypeChange(GeneratedComponent, IProtoFluxEngineProxyNode, IMappableNode, IExecutionNode, INode, ICustomInspector, IObjectRoot, IWorldEventReceiver):
    """Wrapper for [ProtoFluxBindings]FrooxEngine.ProtoFlux.Runtimes.Execution.Nodes.Actions.FireOnTypeChange.

    Category: ProtoFlux/Runtimes/Execution/Nodes/Flow
    """

    COMPONENT_TYPE = "[ProtoFluxBindings]FrooxEngine.ProtoFlux.Runtimes.Execution.Nodes.Actions.FireOnTypeChange"

    def __init__(self, only_for_user: str | INodeObjectOutput[User] | None = None, on_changed: str | ISyncNodeOperation | None = None, value: str | INodeObjectOutput[Type] | None = None, *, component: workers.Component | None = None) -> None:
    ...
    def value(self) -> str | None:
        """Target ID of the Value reference (targets INodeObjectOutput[Type])."""
    ...
    @value.setter
    def value(self, target: str | INodeObjectOutput[Type] | None) -> None:
        """Set the Value reference by target ID or INodeObjectOutput[Type] instance."""
        target_id: str | None = target.id if isinstance(target, INodeObjectOutput) else target  # type: ignore[assignment]
        member = self.get_member("Value")
        if isinstance(member, members.Reference):
            member.targetId = target_id
