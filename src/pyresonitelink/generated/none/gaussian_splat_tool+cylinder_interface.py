...
class GaussianSplatTool+CylinderInterface(GeneratedComponent, IComponent, IWorldEventReceiver):
    ...
    def __init__(self, radius: str | IField[primitives.Float] | None = None, height: str | IField[primitives.Float] | None = None, *, component: workers.Component | None = None) -> None:
    ...
    def height(self) -> str | None:
        """Target ID of the Height reference (targets IField[primitives.Float])."""
    ...
    @height.setter
    def height(self, target: str | IField[primitives.Float] | None) -> None:
        """Set the Height reference by target ID or IField[primitives.Float] instance."""
        target_id: str | None = target.id if isinstance(target, IField) else target  # type: ignore[assignment]
        member = self.get_member("Height")
        if isinstance(member, members.Reference):
            member.targetId = target_id
