...
class SpatialVariableBlendDriver(GeneratedComponent, IComponent, IWorldEventReceiver):
    ...
    def __init__(self, blend_weight: str | IField[np.float32] | None = None, *, component: workers.Component | None = None) -> None:
    ...
    def blend_weight(self) -> str | None:
        """Target ID of the BlendWeight reference (targets IField[np.float32])."""
    ...
    @blend_weight.setter
    def blend_weight(self, target: str | IField[np.float32] | None) -> None:
        """Set the BlendWeight reference by target ID or IField[np.float32] instance."""
