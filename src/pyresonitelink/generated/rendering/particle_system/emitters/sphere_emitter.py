...
class SphereEmitter(GeneratedComponent, IParticleSystemEmitter):
    ...
    @direction_reference_point.setter
    def direction_reference_point(self, value: primitives.Float3) -> None:
        """Set the DirectionReferencePoint field value."""
        member = self.get_member("DirectionReferencePoint")
        if member is not None:
            member.value = value  # type: ignore[attr-defined]
        else:
            self.set_member(
                "DirectionReferencePoint", fields.FieldFloat3(value=value)
            )
