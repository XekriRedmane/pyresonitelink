...
class ConeEmitter(GeneratedComponent, IParticleSystemEmitter):
    ...
    def base_radius(self, value: primitives.Float) -> None:
        ...
        if member is not None:
            ...
            self.set_member(
                "BaseRadius", fields.FieldFloat(value=value)
            )
    ...
    def relative_direction_reference_point(self) -> primitives.Float3 | None:
        """The RelativeDirectionReferencePoint field value."""
        member = self.get_member("RelativeDirectionReferencePoint")
        if member is None:
            return None
        return getattr(member, 'value', None)

    @relative_direction_reference_point.setter
    def relative_direction_reference_point(self, value: primitives.Float3) -> None:
        """Set the RelativeDirectionReferencePoint field value."""
        member = self.get_member("RelativeDirectionReferencePoint")
        if member is not None:
            member.value = value  # type: ignore[attr-defined]
        else:
            self.set_member(
                "RelativeDirectionReferencePoint", fields.FieldFloat3(value=value)
            )
